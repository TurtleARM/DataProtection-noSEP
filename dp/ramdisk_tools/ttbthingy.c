...
#define L1_SECT_DEFPROT      (L1_SECT_AP_URW)  // <- RW in L1 section
...
#define KERNEL_PMAP_SYMBOL      0x803ea974
...
static void generate_ttb_entries(uint32_t ram_base)
{
    ...
    // ram_base is phys kernel base, map kernel image to SHADOWMAP_BEGIN virtual address until SHADOWMAP_END with r/w privileges 
...
int dothingy(vm_address_t slide)
{
    ...
    /* kernel dumped, now find pmap from symbol table (iOS 10). */
    uint32_t kernel_pmap = KERNEL_PMAP_SYMBOL + slide;
...
**/
    ...
    while(ptr < ((uint8_t*)SHADOWMAP_END))
    {
        if(!memcmp(ptr, "\xB0\xF5\xFA\x6F\x00\xF0\xA2\x80", 8)//ios7
         ...
         || !memcmp(ptr, "\xB2\xF5\xFA\x6F\x00\xF0\x96\x80", 8))//ios10
        {
            printf("Patching IOSAESAccelerator enable uid key !\n");
            ptr += 4;
            //*((uint32_t*) ptr) = 0x460c460c;
            *((uint32_t*) ptr) = 0xbf00bf00;
        }
