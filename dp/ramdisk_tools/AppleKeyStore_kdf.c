...
int AppleKeyStore_getPasscodeKey10(const char *passcode, size_t passcodeLen, uint8_t *passcodeKey, uint32_t iter) { // iternum = 50k
    unsigned char salt[20] = {'\x1c', '\x84', '\xce', '\x34', '\xe5', '\xa5', '\x3f', '\xa7', '\xeb', '\xde', '\xce', '\x2d', '\xce', '\x4c', '\xb1', '\x75', '\x35', '\xcd', '\x6f', '\x40'};
    pkcs5_pbkdf2(passcode, passcodeLen, (const char*) salt, 20, passcodeKey, 32, 1);
    return AppleKeyStore_derivation10(passcodeKey, 32, iter);
}

int AppleKeyStore_derivation10(void* data, uint32_t dataLength, uint32_t iter) {
    IOReturn ret;
...
int AppleKeyStore_derivation(KeyBag* keybag, void* data, uint32_t dataLength, uint32_t iter, uint32_t vers) // type = 0, vers = 4
{
    IOReturn ret;
    io_connect_t conn = IOAESAccelerator_getIOconnect();
    memset(in.iv, 0, 16);
    ...
    if (keybag->type & 0x40000000) // false
    {
